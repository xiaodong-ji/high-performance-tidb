...
- Point select 场景测试
```
sysbench --config-file=config oltp_point_select --threads=128 --tables=32 --table-size=100000 run
```

- Update index 场景测试
```
sysbench --config-file=config oltp_update_index --threads=128 --tables=32 --table-size=100000 run
```

- Read only 场景测试
```
sysbench --config-file=config oltp_read_only --threads=128 --tables=32 --table-size=100000 run
```

- Write only 场景测试
```
sysbench --config-file=config oltp_write_only --threads=128 --tables=32 --table-size=100000 run
```

- Read Write 场景测试
```
sysbench --config-file=config oltp_read_write --threads=128 --tables=32 --table-size=100000 run
```
